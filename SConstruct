...
if PLATFORM == 'darwin':
    opts.AddVariables(
        BoolVariable('UNIVERSAL',
                   'Build universal binaries (see UNIVERSAL_ARCHS)', 1),
        BoolVariable('INTERNAL_LIBSNDFILE',
...
# for nova-simd
env.Append(CPPPATH = ["include/nova-simd"])
...
if PLATFORM == 'linux':
    ...
    if env['amd64']:
        ...
        # include libc-macro to include LFS-support on non-64-bit:
        lfs_cflags = os.popen("getconf LFS_CFLAGS").read()[:-1]
        for flag in lfs_cflags.split():
...
if isDefaultBuild():
    ...
    if env['NO_LIBSNDFILE']:
        ...
        if PLATFORM == 'darwin' and env['INTERNAL_LIBSNDFILE']:
            libraries['sndfile'] = Environment(LINKFLAGS = ['../mac/lib/scUBlibsndfile.a'],
                                               CPPPATH = ['#include/libsndfile'])
        else:
...
# libicu
if env['LANG']:
    # nothing needed if darwin, icucore is linked further down with maccy things
    if PLATFORM == 'darwin':
...
env.Alias('install-programs', env.Install(lib_dir(INSTALL_PREFIX), [libscsynth, 'build/libscsynth.so']))
...
langEnv.Append(LIBS = ['common', 'scsynth', 'QtCollider', 'mapper-0'])
...
Source/lang/LangPrimSource/PyrMapperPrim.cpp
...
if env['LANG']:
    langEnv.Append(CPPDEFINES='YYSTACK_USE_ALLOCA')

    libsclang = libsclangEnv.SharedLibrary('build/sclang', libsclangSources)
    env.Alias('install-bin', env.Install(lib_dir(INSTALL_PREFIX), [libsclang, 'build/libsclang.so']))
    if PLATFORM == 'darwin':
